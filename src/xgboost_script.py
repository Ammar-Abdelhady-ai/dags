...
def train_model(X_train, y_train, plot_name, n_estimators: int, learning_rate: float, max_depth: int, scale=None):
    ...
    clf = XGBClassifier(objective="binary:logistic", n_estimators=n_estimators, max_depth=max_depth, 
    ...
    with mlflow.start_run() as run:
        ...
        mlflow.xgboost.log_model(clf, f"{clf_name}/{plot_name}")
...
def main(learning_rate: float, n_estimators: int, max_depth: int):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_estimators", "-n", type=int, default=300)
